...
public final class MusicGroupImpl implements MusicGroup {
    ...
    public Stream<String> albumInYear(final int year) {
        return albums.keySet().stream().filter(t -> albums.get(t) == year);
    }
    @Override
    public int countSongs(final String albumName) {
        return (int)songs.stream().filter(t -> t.getAlbumName().isPresent() && t.getAlbumName().get().equals(albumName)).count();
    }
    ...
    public OptionalDouble averageDurationOfSongs(final String albumName) {
        return songs.stream().filter(t -> t.getAlbumName().map(name -> name.equals(albumName)).orElse(false))
