...
public final class LambdaFilter extends JFrame {
    ...
    private enum Command {
        ...
        LINES_COUNT("Count the number of lines", s -> String.valueOf(s.lines().count())),
        ALPHABETICAL("List all the words in alphabetical order", 
                        s -> Arrays.stream(s.split("(\\s|\\p{Punct})+"))
                                .sorted()
                                .collect(Collectors.joining("\n"))),
        WORD_COUNT("Write the count for each word", 
                        s -> Arrays.stream(s.split("(\\s|\\p{Punct})+"))
